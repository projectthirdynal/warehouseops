...
static CString callsItems[] = {
_T(""),
_T("hold"),
_T("all"),
};
static CString callsValues[] = {
_T("No"),
_T("Hold"),
_T("All Calls"),
};
...
BOOL AAOptionsDlg::OnInitDialog()
{
	...
	CComboBox *combobox;
	combobox= (CComboBox*)GetDlgItem(IDC_AA_OPTIONS_CALLS);
	int n = sizeof(callsItems)/sizeof(callsItems[0]);
	bool found = false;
	for (int i=0;i<n;i++) {
		combobox->AddString(Translate(callsValues[i].GetBuffer()));
		if (accountSettings.autoAnswerCalls==callsItems[i]) {
...
void AAOptionsDlg::OnBnClickedOk()
{
	...
	CComboBox *combobox;
	combobox= (CComboBox*)GetDlgItem(IDC_AA_OPTIONS_CALLS);
	accountSettings.autoAnswerCalls = callsItems[combobox->GetCurSel()];
